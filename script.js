...
// Validateur email simple et robuste
function isValidEmail(v) {
  // On s’appuie d’abord sur la validation native, puis un motif raisonnable
  if (fields.email && fields.email.type === 'email') {
    const tmp = document.createElement('input');
    tmp.type = 'email';
    tmp.value = v;
    if (!tmp.checkValidity()) return false;
  }
  const re = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i;
  return re.test(v);
}
...
form.addEventListener('submit', (e) => {
  ...
  // Tout est OK → masquer le formulaire et afficher le récap
  form.hidden = true;
  summarySection.hidden = false;

  // Construire la data sans mot de passe
  ...
  // Injection sémantique dans <dl>
  recapList.innerHTML = recap
    .map(([k, v]) => `<dt>${k}</dt><dd>${escapeHtml(String(v))}</dd>`)
    .join('');
});

// Bouton “Modifier” pour revenir au formulaire
editBtn.addEventListener('click', () => {
  summarySection.hidden = true;
  form.hidden = false;
  // On laisse les valeurs saisies pour édition
